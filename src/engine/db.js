...
function _migrateLegacy() {
  try {
    ...
    persist()
  } catch { /* silently skip broken data */ }
}
...
export function upsertNote(note) {
  ...
  _db.run(
    ...
    [note.id, note.title ?? '', note.content ?? '', tagsJSON, note.createdAt, note.updatedAt]
  )
...
function _buildFTSQuery(raw) {
  ...
  const words = t.replace(/[^\w\s]/g, ' ').trim().split(/\s+/).filter(w => w.length > 1)
